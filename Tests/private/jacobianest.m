...
function [jac,err] = jacobianest(fun,x0,MaxRelStep)
...
nx = numel(x0);
if numel(MaxRelStep)==1
  MaxRelStep = MaxRelStep*ones(1,nx);
end
...
steps = StepRatio.^(0:-1:-nSteps);
nsteps = length(steps);

% total number of derivatives we will need to take
jac = zeros(n,nx);
err = jac;
for i = 1:nx % loop over all variables
  relativedelta = MaxRelStep(i)*steps;
  x0_i = x0(i);
  if x0_i ~= 0
  ...
  for j = 1:nsteps
    ...
    [fm] = fun(swapelement(x0,i,x0_i - delta(j)));
  ...
  for j = 1:n
    [der_romb,errest] = romberg_extrapolation(StepRatio,derest(j,:),[2 4]);
...
% ============================================
function [der_romb,errest] = romberg_extrapolation(StepRatio,der_init,rombexpon)
